    ...
    async function fetchUsers() {
      try {
        ...
        setUsers(data.users.map((u, idx) => ({
          ...
          status: idx % 3 === 0 ? "Active" : idx % 3 === 1 ? "Pending" : "Blocked",
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-10">
      <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest text-rose-500">Security: Stable</span>
          </div>
          <h1 className="text-3xl font-bold tracking-tight">System Admin</h1>
          <p className="text-zinc-500">Complete control over the platform and infrastructure.</p>
        </div>
        <div className="flex items-center gap-3">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <AdminStat label="Total Users" value="42,891" trend={5.4} icon={Users} color="bg-indigo-500" />
        <AdminStat label="Active Sessions" value="1,204" trend={12.8} icon={Activity} color="bg-emerald-500" />
        <AdminStat label="API Requests" value="1.4M" trend={8.2} icon={Globe} color="bg-blue-500" />
      </div>
        ...
        <div className="lg:col-span-2 bg-white dark:bg-black rounded-3xl border border-zinc-200 dark:border-zinc-800 p-0 overflow-hidden">
          ...
          <table className="w-full">
            ...
            <tbody className="text-sm">
              {users.map((user) => (
                <tr key={user.email} className="border-b border-zinc-50 dark:border-zinc-900/50 hover:bg-zinc-50/50 dark:hover:bg-zinc-900/10 transition-colors">
                  <td className="px-6 py-4">
                    <p className="font-bold">{user.name}</p>
                    <p className="text-xs text-zinc-500">{user.email}</p>
                  </td>
                  <td className="px-6 py-4 font-medium text-zinc-600 dark:text-zinc-400">{user.role}</td>
                  <td className="px-6 py-4">
                    <span className={cn(
                      "px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider",
                      user.status === "Active" ? "bg-emerald-100 text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400" :
                      user.status === "Pending" ? "bg-amber-100 text-amber-600 dark:bg-amber-950/30 dark:text-amber-400" : 
